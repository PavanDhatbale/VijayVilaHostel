...
const MonitorMessages = () => {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setContent('');
                setIsPriority(false);
                setIsComposeModalOpen(false); // Close modal
    ...
    const handleMarkAsRead = async (id) => {
        try {
            ...
            if (response.ok) {
                ...
                // Dispatch event to update header/sidebar notification counts if they listen to it
                // or we rely on page refresh/polling. For now, this updates the view.
    ...
    // Filter messages
    const receivedMessages = history.filter(msg => msg.senderRole === 'hostelManager');
    const sentMessages = history.filter(msg => msg.senderRole === 'monitor');
                    ...
                    <div className="max-w-7xl mx-auto space-y-8">
                        ...
                        <div className="flex justify-between items-end">
                            <div>
                                ...
                                <p className="text-gray-500 font-medium">Communicate with students and hostel manager</p>
                            </div>
                            <button
                        ...
                        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden pb-4">
                            <div className="p-8 border-b border-gray-50 flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h2 className="text-xl font-bold text-gray-900">Inbox (From Manager)</h2>
                                </div>
                            ...
                            <div className="divide-y divide-gray-50">
                                {fetching ? (
                                    ...
                                    <div className="p-12 text-center flex flex-col items-center gap-4">
                                        ...
                                        <p className="text-gray-400 font-medium">No messages received from manager.</p>
                                    </div>
                                ) : (
                                    receivedMessages.map((msg) => (
                                        <div
                                            ...
                                            className={`p-8 transition-all group relative cursor-pointer border-l-4 ${!msg.isRead ? 'bg-blue-50/30 border-blue-500' : 'hover:bg-gray-50/50 border-transparent'}`}
                                        >
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleDeleteNotice(msg._id);
                                                }}
                                            ...
                                            {/* Allow deleting received messages if visually needed, though usually restrict */}
                                            {/* For consistency with previous request, we'll allow delete or keep it simple */}

                                            <div className="flex items-start gap-5">
                                                <div className={`h-12 w-12 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-sm border ${!msg.isRead ? 'bg-blue-100 text-blue-600 border-blue-200' : 'bg-gray-50 text-gray-400 border-gray-100'}`}>
                                                    <UserCircle size={24} />
                                                </div>
                                                <div className="flex-1 space-y-2">
                                                    <div className="flex items-center gap-3">
                                                        <h4 className={`text-lg leading-tight ${!msg.isRead ? 'font-bold text-gray-900' : 'font-semibold text-gray-700'}`}>
                                                            {msg.title}
                                                            {!msg.isRead && (
                                                ...
                                                <div className="flex items-center justify-between border-t border-gray-50 pt-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-gray-900 font-bold text-xs tracking-tight bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
                                                            {msg.recipientType === 'ALL_STUDENTS' ? 'All Students' : 'Hostel Manager'}
                                                        </span>
                                                    </div>
                                    ...
                                    <div className="bg-blue-600 p-8 text-white relative">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p className="text-blue-100">Send updates to students or manager</p>
                                            </div>
                                            <button
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                >
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs font-medium text-gray-400">Broadcast to everyone</p>
                                                    </div>
                                                </button>
                                                <button
                                                    onClick={() => setRecipient('HOSTEL_MANAGER')}
                                                    className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-left ${recipient === 'HOSTEL_MANAGER' ? 'border-blue-600 bg-blue-50/50' : 'border-gray-100 bg-white hover:border-gray-200'}`}
                                                >
                                                    <div className={`p-3 rounded-xl ${recipient === 'HOSTEL_MANAGER' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
                                                        <UserCircle size={24} />
                                                    </div>
                                                    <div>
                                                        <p className="font-semibold text-gray-900">Hostel Manager</p>
                                                        <p className="text-xs font-medium text-gray-400">Direct message to manager</p>
                                                    </div>
                                                </button>
                                            </div>
                                        </div>
